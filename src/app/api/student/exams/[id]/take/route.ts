...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  const existingResult = await Result.findOne({ userId: user.id, examId: params.id });
  if (existingResult) {
  ...
  const questions = await Question.find({ examId: params.id }).select('-correctAnswer -createdAt -updatedAt');
